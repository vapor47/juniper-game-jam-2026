...
func _ready() -> void:
	for i in DebuffPool.ALL.size():
		var debuff: Debuff = DebuffPool.ALL[i].new()
		debuff_option_button.add_item(debuff.display_name, i)
		debuff_option_button.set_item_metadata(i, debuff)
...
func _on_apply_debuff_button_pressed() -> void:
	...
	_apply_debuff(debuff_option_button.get_item_metadata(idx))
